 ...
 * 
 */
public class DevopsLinkSubFiledDialog extends Dialog {
	...
	private DbRes res;
	private CheckboxTreeViewer subFiledtreeViewer;
	...
	String focusInValue = "";
	String focusOutValue = "";

	/**
	 * @wbp.parser.constructor
	 */
	public DevopsLinkSubFiledDialog(Shell shell2, int applicationModal, DbRes res, JarClassLoader classLoader) {
	...
	public DevopsLinkSubFiledDialog(Shell shell2, int applicationModal, RootModel rootModel, String tableName, Table tblColumns,
	...
	public DevopsLinkSubFiledDialog(Shell shell2, int applicationModal, Node element, String tableName, String filedLink, DbRes res,
			String schema) {
		super(shell2, applicationModal);
		this.tableName = tableName;
	...
	private void createContents() {
		...
		GridData gd_txtMainTable = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
		...
		GridData gd_txtMainField = new GridData(SWT.LEFT, SWT.CENTER, false, false, 3, 1);
		...
		GridData gd_txtRefTable = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		...
		GridData gd_txtAsTables = new GridData(SWT.LEFT, SWT.CENTER, false, false, 1, 1);
		...
		GridData gd_comRefField = new GridData(SWT.LEFT, SWT.CENTER, false, false, 4, 1);
		...
		button_2.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				resultMap.put("choseNode", subFiledtreeViewer.getCheckedElements());
		...
		btnSql.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				DevopsTestSqlDialog dialog = new DevopsTestSqlDialog(getShell(), SWT.APPLICATION_MODAL | SWT.CLOSE);
				dialog.setDbRes(res);
				dialog.open();
			}
		});
		...
		txtModifyFiledAsName = new Text(group, SWT.BORDER);
		txtModifyFiledAsName.addFocusListener(new FocusAdapter() {
			@Override
			...
			public void focusLost(FocusEvent e) {
				subFiledFocusOutChek();
			}
		});
		txtModifyFiledAsName.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));

		subFiledtreeViewer = new CheckboxTreeViewer(composite, SWT.BORDER | SWT.CHECK);
		Tree tree = subFiledtreeViewer.getTree();
		GridData gd_tree = new GridData(SWT.FILL, SWT.FILL, true, true, 5, 1);
	...
	private Text txtModifyFiledAsName;
	private Text txtSchema;

	protected void treeAllCheck() {
		if (!isCheck) {
			subFiledtreeViewer.setAllChecked(true);
			isCheck = true;
		} else {
			subFiledtreeViewer.setAllChecked(false);
			isCheck = false;
		}
	...
	protected void chaneList() {
		...
		try {
			...
			subFiledtreeViewer.setInput(root);
	...
	private void initUI() {
		...
		subFiledtreeViewer.setContentProvider(new TVContentProvider());

		subFiledtreeViewer.setLabelProvider(new TVLabelProvider());

		subFiledtreeViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (event.getSelection() instanceof IStructuredSelection) {
					...
					for (Iterator iterator = selection.iterator(); iterator
						...
						txtModifyFiledAsName.setText(fieldValue);
						focusInValue = fieldValue;
		...
		Menu subFiledmenu = new Menu(subFiledtreeViewer.getTree());

		MenuItem deleteItem = new MenuItem(subFiledmenu, SWT.PUSH);
		...
		MenuItem reNameItem = new MenuItem(subFiledmenu, SWT.PUSH);
		...
		subFiledtreeViewer.getTree().setMenu(subFiledmenu);
	 ...
	 * 检查是否
	 */
	private void subFiledFocusOutChek() {
		focusOutValue = txtModifyFiledAsName.getText();
		if (focusInValue != null) {
			if (!focusOutValue.equals(focusInValue)) {
				processRename(null);
			}
		}
	}
