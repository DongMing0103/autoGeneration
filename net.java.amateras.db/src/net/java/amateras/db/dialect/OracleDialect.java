...
public class OracleDialect extends AbstractDialect {
	...
	private static final IColumnType[] COLUMN_TYPES = {
			new ColumnType("NUMBER",
					Messages.getResourceString("type.numeric"), true,
					Types.NUMERIC),
			new ColumnType("INTEGER",
					Messages.getResourceString("type.integer"), true,
					Types.INTEGER),
			new ColumnType("BINARY_FLOAT",
					Messages.getResourceString("type.bit"), false, Types.FLOAT),
			new ColumnType("DOUBLE PRECISION",
					Messages.getResourceString("type.real"), false,
					Types.DOUBLE),
			new ColumnType("VARCHAR2",
					Messages.getResourceString("type.string"), true,
					Types.VARCHAR),
			// new ColumnType("NVARCHAR2",
			// Messages.getResourceString("type.string"),true, Types.NVARCHAR),
			new ColumnType("CHAR", Messages.getResourceString("type.char"),
					true, Types.CHAR),
			// new ColumnType("NCHAR",
			// Messages.getResourceString("type.char"),true, Types.NCHAR),
			new ColumnType("CLOB", Messages.getResourceString("type.string"),
					true, Types.CLOB),
			new ColumnType("LONG", Messages.getResourceString("type.string"),
					false, Types.CLOB),
			new ColumnType("DATE", Messages.getResourceString("type.date"),
					false, Types.DATE),
			new ColumnType("TIMESTAMP",
					Messages.getResourceString("type.date"), true,
					Types.TIMESTAMP),
			new ColumnType("RAW", Messages.getResourceString("type.binary"),
					false, Types.BINARY),
			new ColumnType("BLOB", Messages.getResourceString("type.binary"),
					false, Types.BLOB), };

	public OracleDialect() {
		super(COLUMN_TYPES);
		DbRes.dbStatus.put("oracle", this);
		//MySQL
	...
	public ISchemaLoader getSchemaLoader() {
		OracleSchemaLoader sche = new OracleSchemaLoader();
		DbRes.dbStatus.put("oracleSchema", sche);
		return sche;
	...
	public String createTableDDL(RootModel root, TableModel model,
		...
		String ddl = super.createTableDDL(root, model, schema, drop,
		...
		if (comment) {
			sbComment.append("COMMENT ON TABLE ").append(model.getTableName())
					.append(" is ");
		...
		for (ColumnModel column : model.getColumns()) {
			if (column.isAutoIncrement()) {
				String seqName = model.getTableName() + "_"
						+ column.getColumnName() + "_SEQ";
				String triggerName = model.getTableName() + "_"
						+ column.getColumnName() + "_TRG";
				...
				sb.append("BEFORE INSERT ON ").append(model.getTableName())
						.append(LS);
				sb.append("FOR EACH ROW").append(LS);
				sb.append("BEGIN").append(LS);
				sb.append("IF :NEW.").append(column.getColumnName())
						.append(" IS NOT NULL THEN").append(LS);
				sb.append("  SELECT ").append(seqName).append(".NEXTVAL ");
				sb.append("INTO :NEW.").append(column.getColumnName())
						.append(" FROM DUAL;").append(LS);
				sb.append("END IF;").append(LS);
				sb.append("END;").append(LS);
			}

			if (comment) {
				if (column.getLogicalName() != null
						&& column.getLogicalName().length() > 0) {
					sbComment.append("COMMENT ON COLUMN ")
							.append(model.getTableName()).append(".");
					sbComment.append(column.getColumnName()).append(" is ");
					sbComment.append("'").append(column.getLogicalName())
							.append("';");
					sbComment.append(LS);
				}
			}
