...
public class ForeignKeyModel extends AbstractDBConnectionModel {
	...
	private Map<ColumnModelHtml, ColumnModelHtml> references = new HashMap<ColumnModelHtml, ColumnModelHtml>();
	...
	public ForeignKeyMapping[] getMapping(){
		...
		ColumnModelHtml[] targetColumns = target.getColumns();
		...
		for(int i=0;i<targetColumns.length;i++){
			if(targetColumns[i].isPrimaryKey()){
				...
				ColumnModelHtml referColumn = (ColumnModelHtml) references.get(targetColumns[i]);
				TableModel source = (TableModel) getSource();
				ColumnModelHtml[] sourceColumns = source.getColumns();
