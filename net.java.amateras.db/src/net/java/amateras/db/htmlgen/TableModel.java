...
public class TableModel extends AbstractDBEntityModel {
	...
	private ColumnModelHtml[] columns = new ColumnModelHtml[0];
	...
	public ColumnModelHtml[] getPrimaryKeyColumns(){
		List<ColumnModelHtml> primaryKeyColumns = new ArrayList<ColumnModelHtml>();
		for(ColumnModelHtml columnModel: getColumns()){
			if(columnModel.isPrimaryKey()){
				primaryKeyColumns.add(columnModel);
			}
		}
		return primaryKeyColumns.toArray(new ColumnModelHtml[primaryKeyColumns.size()]);
	}
	...
	public void setColumns(ColumnModelHtml[] columns){
		this.columns = columns;
	}

	public ColumnModelHtml[] getColumns(){
		return this.columns;
	}

	public ColumnModelHtml getColumn(String columnName){
