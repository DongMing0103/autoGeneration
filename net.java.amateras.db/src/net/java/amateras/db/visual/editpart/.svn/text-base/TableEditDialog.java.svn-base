...
import cn.devops.controller.DevopsMainTableDialog;
...
public class TableEditDialog extends Dialog {
	...
	private Button btnDelColumnMutil;
	 ...
	 *            SQL
	 */
	public TableEditDialog(Shell parentShell, RootModel rootModel, String tableName, String tableLogicalName, String tableDescription,
			ColumnModel[] columns, ColumnModel editColumn, IndexModel[] indices, boolean indexEditing, IndexModel editIndex, String sql) {
		super(parentShell);
	...
	protected void constrainShellSize() {
		...
		shell.pack();
		shell.setSize(shell.getSize().x, 650);
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		tblColumns = new Table(tableArea, SWT.CHECK | SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION | SWT.V_SCROLL | SWT.H_SCROLL);
		...
		btnDelColumn.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				// TableItem[] tableSelect = tblColumns.getSelection();

				// int index[] = tblColumns.getSelectionIndices();
		...
		btnDelColumnMutil = new Button(buttons, SWT.PUSH);
		btnDelColumnMutil.setText("mutil del");
		btnDelColumnMutil.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		btnDelColumnMutil.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
		...
		btnGenCode = new Button(buttons, SWT.PUSH);
		btnGenCode.setText("代码生成");
		btnGenCode.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		...
		GridLayout layoutnIndex = new GridLayout(2, false);
		layoutnIndex.horizontalSpacing = 0;
		layoutnIndex.verticalSpacing = 0;
		layoutnIndex.marginHeight = 0;
		layoutnIndex.marginWidth = 0;

		Composite indexArea = new Composite(composite3, SWT.NULL);
		indexArea.setLayout(layoutnIndex);
		...
		GridLayout indexsLayout = new GridLayout(1, false);
		indexsLayout.horizontalSpacing = 0;
		indexsLayout.verticalSpacing = 0;
		indexsLayout.marginHeight = 0;
		indexsLayout.marginWidth = 2;

		Composite indexButtons = new Composite(indexArea, SWT.NULL);
		indexButtons.setLayout(indexsLayout);
		...
		GridLayout indexColLayout = new GridLayout(1, false);
		indexColLayout.horizontalSpacing = 0;
		indexColLayout.verticalSpacing = 0;
		indexColLayout.marginHeight = 0;
		indexColLayout.marginWidth = 2;

		Composite indexColumnButtons = new Composite(indexColumn, SWT.NULL);
		indexColumnButtons.setLayout(indexColLayout);
		...
		IDocumentPartitioner partitioner = new FastPartitioner(new SQLPartitionScanner(), new String[] { SQLPartitionScanner.SQL_COMMENT,
	 ...
	 */
	public  void openGen() {
		DbRes res = DbRes.getInstance(rootModel.getJdbcUrl());
		res.setRoot(rootModel);

		DevopsMainTableDialog genCodeMainDialog = new DevopsMainTableDialog(this.getShell(), SWT.APPLICATION_MODAL | SWT.MAX, rootModel, tableName,
				tblColumns, columnModels, res, txtTableLogicalName.getText());
		genCodeMainDialog.open();
	}
	...
	private void updateColumn() {
		if (editColumnIndex != -1 && cmbColumnType.getSelectionIndex() != -1) {
			...
			if (columnIndex < rootModel.getDommains().size()) {
				...
				model.setColumnType(dialect.getColumnTypes()[cmbColumnType.getSelectionIndex() - rootModel.getDommains().size()]);
			}
