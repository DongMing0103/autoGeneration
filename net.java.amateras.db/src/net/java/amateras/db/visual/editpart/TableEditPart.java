...
public class TableEditPart extends AbstractDBEntityEditPart implements
	...
	private ColumnFigure[] createColumnFigure(RootModel root, TableModel table,
		...
		if (root.getLogicalMode()) {
			label1.setText(model.getColumnNameOut());
		} else {
			label1.setText(model.getColumnName());
		}
		label1.setUnderline(model.isPrimaryKey());
		if (root.getLogicalMode()) {
			if (model.getLogicalName().length() > 10) {
				label2.setText(sb.toString() + "   "
						+ model.getLogicalName().substring(0, 10));
			} else {
				label2.setText(sb.toString() + "   " + model.getLogicalName());
			}
		}else{
			label2.setText(sb.toString());
		}
			...
			if (obj instanceof ForeignKeyModel) {
				...
				for (int j = 0; j < mappings.length; j++) {
					if (mappings[j].getRefer() != null
							&& mappings[j].getRefer().getColumnName()
		...
		if (model.isNotNull()
				&& DBPlugin.getDefault().getPreferenceStore()
						.getBoolean(DBPlugin.PREF_SHOW_NOT_NULL)) {
			if (root.getLogicalMode()) {
	...
	public static void openTableEditDialog(EditPartViewer viewer,
		...
		TableEditDialog dialog = new TableEditDialog(viewer.getControl()
				...
				model.getLogicalName(), model.getDescription(),
				model.getColumns(), editColumn, model.getIndices(), false,
				null, model.getSql());
			...
			viewer.getEditDomain()
					...
					.execute(
							new TableEditCommand(model, dialog.getTableName(),
									dialog.getTableLogicalName(), dialog
											.getTableDescription(), columns
											.toArray(new ColumnModel[columns
													.size()]), indices
											.toArray(new IndexModel[indices
													.size()]), dialog.getSql()));
		}
	}
	...
	public static void openTableEditDialog(EditPartViewer viewer,
		...
		TableEditDialog dialog = new TableEditDialog(viewer.getControl()
				.getShell(), root, model.getTableName(),
				model.getLogicalName(), model.getDescription(),
				model.getColumns(), null, model.getIndices(), true, editIndex,
				model.getSql());
