...
public class TableModel extends AbstractDBEntityModel implements ICloneableModel {
	...
	private ColumnModel[] columnsMain = new ColumnModel[0];
	/**
	 * 索引
	 */
	...
	public static final String P_COLUMNSMain = "p_columnsMain";
	...
	public void setTableName(String tableName) {
		...
		for (int i = 0; i < arr.length; i++) {
			if (i == 0) {
				table += arr[i].substring(0).toLowerCase();
			} else {
				table += arr[i].substring(0, 1).toUpperCase() + arr[i].substring(1).toLowerCase();
			}
		}
		this.tableNameOut = table;
	...
	public String getTableNameOut() {
		if (tableNameOut != null && !"".equals(tableNameOut)) {
			return tableNameOut.substring(0, 1).toUpperCase() + tableNameOut.substring(1);
		}
		return this.tableNameOut;
	}

	public String getTableNameOutLow() {
	...
	public void setColumnsMain(ColumnModel[] columnsMain) {
		this.columnsMain = columnsMain;
		firePropertyChange(P_COLUMNSMain, null, columnsMain);
	}

	public void setColumnsArr(Object[] array) {
		ColumnModel[] colArr = new ColumnModel[array.length];
		for (int i = 0; i < array.length; i++) {
			colArr[i] = (ColumnModel) array[i];
		}
		setColumns(colArr);

	}

	public void setColumnsMainArr(Object[] array) {
		ColumnModel[] colArr = new ColumnModel[array.length];
		for (int i = 0; i < array.length; i++) {
			colArr[i] = (ColumnModel) array[i];
		}
		setColumnsMain(colArr);
	...
	public ColumnModel[] getColumnsMain() {
	...
	public String getAllTableNameAs() {
		return joinSqlBuff.toString();// tableName + " " + tableNameAs;
	}

	StringBuffer joinSqlBuff = new StringBuffer();

	public void appSqlTableJoin(String appData) {
		joinSqlBuff.append(appData);
